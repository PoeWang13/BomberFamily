...
public class Board_Home : Board_Object
{
    ...
    private void Update()
    {
        ...
        if (spawnTimeNext > spawnTime)
        {
            ...
            SendBossOutSide(enemy_Base.transform, rndDir);
            enemy_Base.transform.SetParent(boardEnemyParent);
        }
    }
    private void SendBossOutSide(Transform board_Object, int rndDirec)
    {
        // Bossu dışarı çıkar
        board_Object.DOMoveY(0, 1.0f).OnComplete(() =>
        {
            // Bossu aşağı yürüt
            int rndDirec = Random.Range(0, boardCoors.Count);
            board_Object.DOMove(new Vector3(boardCoors[rndDirec].x, 0, boardCoors[rndDirec].y), 1.0f).OnComplete(() =>
            {
                // Bossu serbest bırak
